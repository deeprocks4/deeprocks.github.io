...
var config = {
  ...
  redirect_uri: 'https://deeprocks4.github.io',
	...
	var innerNode;
      innerNode = el.p(
        {className: 'navbar-text'},
			  ...
			  el.p(null, "In order to play you will need a balance.  You can use the free faucet to try out some bets for free or you can fund your MoneyPot account.  You will need to sign-up for a free account with MoneyPot in order to play here.  After you have created an account you add the InvestDice casino app to you MoneyPot account.  There are two methods available to add funds at this point.  The more direct way is to deposit directly to your Investdice account by clicking the deposit button at the top of the page and using the deposit address located there. The second is under your MoneyPot account page, you can find the deposit button to generate a new BTC deposit address.    Once your account is funded you can click on deposit from inside the app to bring coins over to play with.  Deposits are available to you after 1 confirmation."),
			  el.p({className: 'lead'}, "Can I play for free?"),
			  el.p(null, "Yes.  There is a free faucet available and can be used every 2 minutes to add a small amount of free coins to you account to play with."),
			  ...
			  el.p({className: 'lead'}, "Chatbot:"),
			  el.p(null, "For your convenience a chatbot is available to check on coin price, compare coin values, alert users to new blocks and a flexible randomizer to help you make decisions.  A list of Chatbot commands can be called at anytime by sending !help in chat.Commands: !price [coin] | !block btc | !c [params] | !rand [params] For more detailed instructions on each command, type the command without any parameters."),
			  el.p({className: 'lead'}, "Tipping users:"),
			  el.p(null, "If you wish to help someone out you can tip other users within Investdice without having to withdraw your coins to Moneypot first.  Simply type /tip [username] [amount] and the coins will be transferred instantly.  If the receiver wishes to see his balance update immediately they will have to click on the refresh balance button on top otherwise it will update automatically after 5 minutes.  Please use caution when tipping users and do not loan coins to those you do not trust."),
